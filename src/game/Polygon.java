...
//    for (Point p : shape) {
  ...
  public int[] getXPoints(Point[] inShape) {
	  int[] xPoints = new int[inShape.length];
	  for (int i = 0; i < inShape.length; i++) {
		  xPoints[i] = (int)inShape[i].getX();
	  }
	  return xPoints;
  }
  public int[] getYPoints(Point[] inShape) {
	  int[] yPoints = new int[inShape.length];
	  for (int i = 0; i < inShape.length; i++) {
		  yPoints[i] = (int)inShape[i].getY();
	  }
	  return yPoints;
  }
  public void addLength() {
	  shape[1].setX(shape[1].getX()+20);
	  shape[2].setX(shape[2].getX()+20);
	  for (int i = 0; i < shape.length; i++) {
  ...
  public boolean checkCollision(Polygon p) {
	  Point[] x = this.getPoints();
